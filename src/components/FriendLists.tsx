...
const FriendLists = (firendLists: any) => {
  return (
    <div className="flex w-full flex-col gap-2 bg-background mt-6 rounded-lg p-2 px-4">
      <div className="w-full justify-center font-rubik">
      ...
      {firendLists.firendLists.length > 0 ? (
        firendLists.firendLists.map((friend: any, index: number) => (
          <div
            key={index}
            className="flex w-full justify-between items-center gap-2 font-rubik text-black font-semibold"
          >
        ...
        <p className="text-black text-lg text-center font-rubik">No friend</p>
      )}
    </div>
  );
};
